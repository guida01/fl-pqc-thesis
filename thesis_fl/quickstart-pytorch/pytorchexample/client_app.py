...
def train_fn(msg: Message, context: Context):
    ...
    if scheme == "no_signature":
        ...
        sig_size = 0
        pubkey_size = 0
        signed_payload_size = 0.0
    else:
        # Experimental policy: generate a fresh signing keypair for each
        # client update.
        sig_mgr = SignatureManager(scheme)
        keygen_time = sig_mgr.keygen_time

        # Serialize exactly the data covered by the signature.
        t0 = time.perf_counter()
        payload = (
        ...
        signed_payload_size = float(len(payload))
        ...
        # underlying key bytes may still linger in memory until the interpreter
        # reuses that space.
        del sig_mgr
